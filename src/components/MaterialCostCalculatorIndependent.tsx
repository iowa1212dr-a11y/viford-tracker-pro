...
import { Calculator, Plus, Trash2, Package, DollarSign, ArrowRightLeft } from "lucide-react";
...
export const MaterialCostCalculator = ({ products }: MaterialCostCalculatorProps) => {
  ...
  const [tempRate, setTempRate] = useState("36.5");
  ...
  const convertAmount = (amount: number, fromCurrency: 'USD' | 'Bs.' = 'USD') => {
  ...
  const totalProductValue = products.reduce((sum, product) => {
    const productPrice = convertAmount(product.total, 'USD');
  ...
  useEffect(() => {
    const savedData = localStorage.getItem('viford-costs-independent');
    if (savedData) {
      ...
      setExchangeRate(data.exchangeRate || 36.5);
      setTempRate((data.exchangeRate || 36.5).toString());
    }
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('viford-costs-independent', JSON.stringify(costData));
  }, [materials, laborCost, overhead, notes, currency, exchangeRate]);
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="p-6">
          {/* Configuración de Moneda para Costos */}
          <div className="mb-6 p-4 bg-accent/5 border border-accent/20 rounded-lg">
            ...
            <div className="space-y-3">
              <div>
                <Label className="text-sm font-medium mb-2 block">Moneda para Costos</Label>
